...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    mapping(address => mapping(uint => uint)) private tracker;

     mapping(address => mapping(uint => bool)) private _checker; 
    ...
    function _safeTransferFrom(
        ...
        if (check==false && _balances[id][to]>0 ){
             tracker[to][id]=block.timestamp;      
        }
    ...
    function _mint(
        ...
        _balances[id][to] += amount;
        tracker[msg.sender][id]=block.timestamp;
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _balances[ids[i]][to] += amounts[i];
             ...
             tracker[msg.sender][ids[i]]=block.timestamp;
        }
